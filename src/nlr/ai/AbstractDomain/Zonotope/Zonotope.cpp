...
void AI::Zonotope::joinWith(AI::AbstractDomain& other)
{
	...
	std::cout << res.getSizeString() << std::endl;
...
AI::AbstractDomainDimBound AI::Zonotope::getBoundsForDim(unsigned dim) {
	...
	return AI::AbstractDomainDimBound(lower, upper);
}
...
void AI::Zonotope::meetGt0(unsigned dim)
{
	...
	for (unsigned j = 0; j < generators.size(); j++) {
		...
		if (canChangeLb) {
			AI::ZonotopeVertex* closest = new AI::ZonotopeVertex(arma::mat(bias.size(), 1, arma::fill::zeros), arma::mat(generators.size(), 1, arma::fill::zeros));
