...
public:
    ...
    void initAsFirstLayer(double **firstLayerBounds) {
        ...
        ap_lincons1_array_t constraintArray = ap_lincons1_array_make(_manager->_env, _manager->_layerSizes[0]);
        ...
        for(unsigned i = 0; i < _manager->_layerSizes[0]; i++) {
            ...
            ap_linexpr1_t exprLb = ap_linexpr1_make(_manager->_env, AP_LINEXPR_SPARSE, 1);
            ap_lincons1_t consLb = ap_lincons1_make(AP_CONS_SUPEQ, &exprLb, NULL);
            ap_lincons1_set_list(&consLb,
                AP_COEFF_S_INT, 1, varName,
                AP_CST_S_INT, -firstLayerBounds[i][0],
                AP_END);
            ap_lincons1_array_set(&constraintArray, i, &consLb);

            ap_linexpr1_t exprUb = ap_linexpr1_make(_manager->_env, AP_LINEXPR_SPARSE, 1);
    ...
    AbstractValue *performAffineTransformation(double **weights, double *bias) {
        ...
        for(unsigned i = 0; i < nextLayerSize; i++ ){
            ...
            sprintf(varName, "x_%d_%d", _layerIndex, i);
            ...
            for(unsigned j = 0; j < currLayerSize; j++) {
                ...
                sprintf(varName, "x_%d_a_%d", _layerIndex-1, j);
                ap_lincons1_set_list( &cons,
                                      AP_COEFF_S_DOUBLE, weight, varName,
                                      AP_END);
            }
    ...
    AbstractValue *performRelu() {
        ...
        for(unsigned i = 0; i < layerSize; ++i) {
            ...
            */
            ...
            ap_abstract1_t *inactiveAV = addr(ap_abstract1_meet_lincons_array(_manager->_manager, false, currentAV, &inactiveConstraintArray));
            ...
            if(ap_abstract1_is_bottom(_manager->_manager, inactiveAV)) {
                inactiveAV = addr(ap_abstract1_bottom(_manager->_manager, _manager->_env));
            }
            else {
                inactiveAV = addr(ap_abstract1_meet_lincons_array(_manager->_manager, false, inactiveAV, &inactiveConstraintArray));
            }
            ...
            /*
                Join the results
            */
            currentAV = addr(ap_abstract1_join( _manager->_manager, false, &activeAV, inactiveAV));
