...
int optpr_cramer(t1p_internal_t* pr, itv_t lambda, itv_t u0, itv_t * l1, itv_t * l2)
{
	...
	if (itv_is_eq(l1[2],l2[2])) {
	    ...
	    /* l1 // l2 avec exactement m�me vecteur directeur */
	    return -1;
	}
    } else {
...
void optpr_solve(t1p_internal_t* pr, itv_t alpha0x, itv_t alpha0, itv_t midgx, itv_t midgy, itv_t taux, itv_t tauy, t1p_aff_t* res)
{
    ...
    /* complexit� (opt->sizeJ)^3 */
    for (i=0; i<opt->sizeJ; i++) {
	for (j=i+1; j<opt->sizeJ; j++) {
	    /* TODO: pour le moment on ne fait rien si deux lignes sont identiques: � optimiser */
		...
		if (optpr_isfeasible(pr, lambda, u0)) {
		    ...
		    */
			...
			/* d�finitivement pas bon - je le mets surtout pour le else */
		    } else if (itv_is_leq(f,opt->optval)) {
			/* do nothing, le optval contient deja f */
		    } else {
			/* entrelacements des intervalles: prendre celui qui a la borne sup la plus grande et garder le point quand m�me */
    ...
    itv_sub(ulambda, un, lambda);
    /* on a la derni�re solution dans opt->optsol et sa valeur dans opt->optval */
